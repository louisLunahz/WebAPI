...
namespace LouigisSP.SL
{
    public class ProductOperations
    {
        ...
        public IEnumerable<Product> GetProductsThatMatch(string pattern)
        {
           IEnumerable <Product> products= new List<Product>();
            var patternP = DBManager.CreateParameter("@pattern","%"+pattern+"%", DbType.String);
            IDbDataParameter[] parameters = new IDbDataParameter[1];
            parameters[0] = patternP;
            DataTable dataTable = DBManager.GetDataTable("SELECT * FROM products WHERE products.name LIKE @pattern or products.brand LIKE @pattern or products.extra_info like @pattern", CommandType.Text, parameters);
            return products = dataTable.AsEnumerable().Select(
                row => new Product
                {
                    id = Convert.ToInt32(row["id"]),
        ...
        public Product GetProductById(int id)
        {
            ...
            if (dataTable.Rows.Count < 1) throw new ElementNotFoundException();
            else
            {
        ...
        public Product EditProduct(int id, Product product)
        {
            if (product == null)
                throw new ArgumentNullException();
            if (product.id != id)
                throw new InvalidDataException("Id's don´t match");
